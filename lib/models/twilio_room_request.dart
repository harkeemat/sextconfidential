...
class TwilioRoomRequest {
  ...
  factory TwilioRoomRequest.fromMap(Map<String, dynamic> data) {
    return TwilioRoomRequest(
      ...
      statusCallbackMethod: EnumToString.fromString(
          TwilioStatusCallbackMethod.values,
          data['statusCallbackMethod'].toString().camelCase),
      type: EnumToString.fromString(
              TwilioRoomType.values, data['type'].toString().camelCase) ??
          TwilioRoomType.peerToPeer,
      uniqueName: data['uniqueName'],
      videoCodecs: (data['videoCodecs'] as List<String>)
          .map(
            (String videoCodec) => EnumToString.fromString(
                TwilioVideoCodec.values, videoCodec.toString().camelCase),
          )
          .toList(),
    );
  }
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'mediaRegion': mediaRegion != null
          ? EnumToString.convertToString(mediaRegion)
          : null,
      'recordParticipantsOnConnect': recordParticipantsOnConnect,
      'statusCallback': statusCallback,
      'statusCallbackMethod': mediaRegion != null
          ? EnumToString.convertToString(statusCallbackMethod)
          : null,
      'type': EnumToString.convertToString(type).paramCase,
      'uniqueName': uniqueName,
      'videoCodecs': videoCodecs
          ?.map((TwilioVideoCodec? videoCodec) =>
              EnumToString.convertToString(videoCodec).toString().camelCase)
          .toList()
    };
  }
}
